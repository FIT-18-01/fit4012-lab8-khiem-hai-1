...
def main() -> None:
    ...
    plaintext, integrity_ok = open_receiver_payload(packet, receiver_private_key, SENDER_PUBLIC_KEY)


    message = plaintext.decode("utf-8", errors="replace")
    if integrity_ok:
        lines.append("[+] Dữ liệu nguyên vẹn: SHA-256 khớp và chữ ký số hợp lệ.")
        print(lines[-1])
    else:
        lines.append("[!] Dữ liệu bị thay đổi hoặc giả mạo: SHA-256 không khớp hoặc chữ ký số không hợp lệ.")
        print(lines[-1])

    # In log theo yêu cầu giảng viên (chữ ký số)
    if integrity_ok:
